...
# bridge bound to localhost to get to the admin port
ACA_PY_IP=localhost
# Make sure ACA-py container gets terminated when we do
trap 'docker kill ${ACA_PY_CONTAINER_ID}' EXIT
waitActiveWebInterface "http://${ACA_PY_IP}:${ACA_PY_ADMIN_PORT}" 20
...
curl --output ${OPEN_API_SHARED_DIR}/acapy-raw.json http://${ACA_PY_IP}:${ACA_PY_ADMIN_PORT}/api/docs/swagger.json
