...
export interface SchemasSchemaIdWriteRecordPostRequest {
  schemaId: string;
}
...
export class SchemaApi extends runtime.BaseAPI {
   ...
   */
  async schemasSchemaIdWriteRecordPostRaw(
    requestParameters: SchemasSchemaIdWriteRecordPostRequest,
  ): Promise<runtime.ApiResponse<SchemaGetResult>> {
    if (
      requestParameters.schemaId === null ||
      requestParameters.schemaId === undefined
      ...
      throw new runtime.RequiredError(
        'schemaId',
        'Required parameter requestParameters.schemaId was null or undefined when calling schemasSchemaIdWriteRecordPost.',
      );
    }
    ...
    const response = await this.request({
      path: `/schemas/{schema_id}/write_record`.replace(
        `{${'schema_id'}}`,
        encodeURIComponent(String(requestParameters.schemaId)),
      ),
      method: 'POST',
   ...
   * Writes a schema non-secret record to the wallet
   */
  async schemasSchemaIdWriteRecordPost(
    requestParameters: SchemasSchemaIdWriteRecordPostRequest,
  ): Promise<SchemaGetResult> {
    const response =
      await this.schemasSchemaIdWriteRecordPostRaw(requestParameters);
    return await response.value();
  }
}
